...
| doc | status | verdict |
|---|---|---|
| [`v4-flash-port.md`](../investigations/v4-flash-port.md) | live | The staged plan to make V4-Flash decode. Corrects other-models.md on three points measured from the downloaded repo: experts are 148.25 GB native FP4 (138.1 GiB) so it DOES stream at ~83% residency, not "nearly fully resident"; 4.02 GB/token not 3.1; the partial fp8 KV act_quant is mandatory, not a --kv-fp8 to refuse; YaRN is per-layer, keyed to compress_ratio. DSpark/MTP is separable and out of scope. |
