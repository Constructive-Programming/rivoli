...
2. **Per-layer RoPE** — **HALF DONE 2026-08-12.** The numeric half is settled and the
   conversion half is not.

   **No new kernel, and no un-permutation either.** §6 proposed permuting `q_proj`/`k_proj` rows
   so rivoli's interleaved `rope_interleave` computes Glimmer's split-half rotation, and expected
   to undo the permutation afterwards. It does not need to: the kernel **reads `(2j, 2j+1)` and
   writes `(j, half+j)`**, so with `P` on the input it emits split-half RoPE already in split-half
   layout. Half of `P` is the kernel's write pattern; the conversion supplies the other half.

   | | `max|Δ| / max|reference|` |
   |---|---:|
   | the permutation, 168 cases (both draws, q and k, every roped layer) | 1.41e-7 |
   | **the tolerance** (`rope` row) | **4.77e-5** |
   | identity in its place, weakest of 84 cases | 9.98e-1 |

   34× under the fp32 floor, against a red proof four decades the other side. §6 is marked
   SETTLED in place with the arithmetic. `tests/glimmer_rope.rs`.
   ...
   **The `rope` tolerance row was measured for this, not borrowed.** Floor 4.773e-6 from
   ...
   1.811e0 and the margin is 379,000×. Both are counted: unlike `attend`'s `qk_scale_on_k` there
   is no algebraic identity hiding either from a rope kernel. The regeneration also reproduced the
   two vendored clean goldens **byte-identically**, which re-checks the anchor's reproducibility
   claim from scratch.

   **The global-layer rule is asserted positively**, as K3's `mla_use_nope` lesson requires: a
   layer is roped iff `layer_rope_theta[l] != 0`, the golden's `layer_is_roped` is cross-checked
   against the presence of a `.roped` capture in both directions, and against `layer_is_sliding`
   — §2's claim that the NoPE layers ARE the full-attention layers, from two independently
   captured tables.

   **What is left.** The fixture permutes a captured ACTIVATION; the shipped plan permutes the
   WEIGHT ROWS. §6's three reasons those are equivalent (`v_proj` is never rotated, `gate_proj`
   acts on the attention output, `qk_norm` commutes with a within-head permutation) stay an
   argument until `convert_glimmer` emits a permuted checkpoint and a golden scores it. That, and
   wiring the rotation into a layer loop, is the rest of item 2.
