...
1. **GQA attend** — the new family. **DONE 2026-08-12**: `gqa_attend` in `kernels/attn.hip`,
   ...
   **What it measured**, over both goldens, all 8 layers, all 7 steps — 112 cases, plus 72 ring
   cases, 8 launcher guards and a width sweep, under the row's own metric
   (`max|Δ| / max|reference|`, which is what `by_operator` computes):

   | | |
   |---|---:|
   | this kernel, cache indexed by position | 8.93e-7 |
   | this kernel, ring-indexed | 3.77e-7 |
   | the host oracle in the test | 4.24e-7 |
   | **the tolerance** | **1.64e-4** |
   | the interleaved KV broadcast, for contrast | 5.62e-1 |

   So the kernel sits **18× under the floor** the tolerance is built on and 184× under the
   tolerance itself, while the defect the gate exists to catch is six decades the other side. It
   was NOT scored this way at first: the original gate measured an absolute difference against a
   bar taken from the kernel's own output, which is defensible alone and incomparable to every
   other number in this port. That is what the pre-measured row is for.
...
2. **Per-layer RoPE** — **DONE 2026-08-12**, and not the way this plan expected.

   **`rope_split_half` in `kernels/linalg.hip`, four lines, sharing its arithmetic with
   `rope_interleave` through one `__device__` helper.** §6 proposed avoiding a new kernel by
   permuting `q_proj`/`k_proj` rows at conversion time. That route was MEASURED WORKING (1.41e-7,
   `tests/glimmer_rope.rs` keeps it) and then declined, on a cost §6 did not price: the permutation
   takes those two tensors out of `copy_verbatim`, so ~3 GB of the artifact stops being a borrowed
   mapping of the checkpoint and **G1a's "round-trips bit-exact" clause stops covering them**.

   | route | `max|Δ| / max|reference|` |
   |---|---:|
   | `rope_split_half`, no permutation anywhere — **shipped** | 1.41e-7 |
   | permute + `rope_interleave` — §6's route, measured, declined | 1.17e-7 |
   | **the tolerance** (`rope` row) | **4.77e-5** |
   | `rope_interleave` on unpermuted input — trap 9 | 9.98e-1 |

   34× under the fp32 floor, against a trap four decades the other side.

   **Two entry points, not one kernel with a flag.** Trap 9 is applying one convention where the
   other is meant; it is fluent and silent, and a `bool split_half` would put it one argument away
   from every existing GLM and V4 call site. `common.hpp`'s `swiglu`/`swiglu_clamped_bf16` note is
   the precedent and it is cited at both kernels. jscpd does not scan `kernels/`, so nothing would
   have caught a copy-paste sibling — the shared helper is a choice, not a gate's demand.

   **The `rope` tolerance row was measured for this, not borrowed.** Floor 4.773e-6 from
   `anchor.md`; both targeting defects regenerated at both draws on CPU and scored —
   `rope_interleaved` 2.505e0/2.214e0, `rope_on_nope_layers` 2.011e0/1.811e0 — so the weakest is
   1.811e0 and the margin is 379,000×. Both count: unlike `attend`'s `qk_scale_on_k` there is no
   algebraic identity hiding either from a rope kernel. That regeneration also reproduced the two
   vendored clean goldens **byte-identically**, re-checking the anchor's reproducibility claim.

   **The global-layer rule is asserted positively**, per K3's `mla_use_nope` lesson: a layer is
   roped iff `layer_rope_theta[l] != 0`, `layer_is_roped` is cross-checked against the presence of
   a `.roped` capture in both directions, and against `layer_is_sliding` — §2's claim that the NoPE
   layers ARE the full-attention layers, from two independently captured tables.

   **What is left for S3, not for this item:** nothing rotates yet in a layer loop, and the
   per-layer θ has no caller. The kernel takes θ as a parameter, so the per-layer table is a
   call-site concern.

3. **Output gate** — **DONE 2026-08-12.** `sigmoid_gate` in `kernels/fwd.hip`, beside `vadd`:
   `x[i] *= sigmoid(g[i])`, applied between the attend and `o_proj`. `gate_proj` itself is an
   ordinary `[4096, 6656]` projection and needs no new kernel.

   | | `max|Δ| / max|reference|` |
   |---|---:|
   | the kernel, 112 cases | 1.58e-7 |
   | **the tolerance** (`o_proj` row) | **8.29e-5** |
   | gating on the attention output — trap 4, weakest | 1.67e-1 |

   53× under the fp32 floor, against a trap 2,010× the other side. The `o_proj` row was measured
   for this rather than borrowed: floor 8.293e-6, `gate_disabled` at 3.759e0/3.688e0 by draw, so
   the weakest is 3.688e0 and the margin is 444,700×.

   **The trap is the operand and the fixture says so out loud.** §4 item 3: `gate_proj` consumes
   the LAYER INPUT, not the attention output. `tests/glimmer_gate.rs` runs that substitution as a
   red proof, and carries a third test asserting the two operands are far apart everywhere —
   because the trap is only detectable here while they are, and that is a property of the
   fixture rather than of the kernel.

   **A separate launch, not folded into `gqa_attend`'s output write.** Folding saves one pass over
   `[rows][4096]` and has no measurement behind it; it would also put a second operand inside a
   kernel whose fixture currently means "the attention is right". S5 prices it.

   **`expf`, not `__expf`.** The fast intrinsic is a different function with its own error, and
   taking it would price this tolerance against an approximation nobody measured.

   > **CORRECTED 2026-08-12.** This item said "K3's gated-MLA kernel pattern at the GQA site",
   > which points at nothing: **Kimi-K3 is at S1b and has no kernels in the tree** — its gated MLA
   > is item 2 of its own unwritten S2 list, and the launcher census carries zero K3 rows. The two
   ...
   > multiply of two `[rows][32*128]` tensors. What they share is one sentence of spec, not a
...
4. **MLP wiring** — **DONE 2026-08-12, and two of its three clauses were already true.**
   ...
   **What was missing is that nothing ran them at 6656/19968.** Every golden is at the tiny
   widths, so `132,907,008` elements per weight matrix had never been touched — the same hole
   ...
   | | `max|Δ| / max|reference|` |
   |---|---:|
   | `gate`/`up` `[19968 x 6656]` | 2.03e-6 |
   | `down` `[6656 x 19968]` | 2.06e-6 |
   | the composition | 4.04e-6 |
   | swapping `gate` and `up` ON THE DEVICE — the red proof | 4.76e-1 |

   The bars are ~24× the measured worst and are **not** derived from an anchor floor: the anchor
   ...
   > **CORRECTED 2026-08-12, review round 2 — this table's first numbers were measured on a
   > fixture that could not see what it claimed.** Four findings, all fixed the same day:
   > the element count read `132,913,152` (off by exactly 6,144 = GLM's hidden size, a
   > copy-paste tell; the claim built on it — "past 2^27, i32-overflow coverage" — was doubly
   > false, since the true product is below 2^27 and 16x under `i32::MAX`); the shared `fill`
   > was linear mod 65536, so at row stride 6656 the weight matrices held only **128 distinct
   > rows** (gcd(6656, 65536) = 512) and a kernel reading row c+128 for row c would have passed
   > bit-identically — first numbers 3.27e-6/5.17e-6, re-measured 2.03e-6/2.06e-6 after a
   > mixing xor; the swap red proof compared `host(false)` to `host(true)` — two HOST
   > computations, proving the metric discriminates and nothing about the device (now
   > `swiglu(&u, &g)` on the device, 4.76e-1); and the spot metric was a per-element ratio
   > incomparable with `worst_rel` — the absolute-metric mistake item 1 corrected, one file
   > over. The spot stride was also even at both widths, so 63 of 64 samples were even-indexed
   > while the comment claimed coprimality.
...
5. **lm_head at 202048 + the logit path** — **DONE 2026-08-12. Three of this item's own claims
   were wrong, and the third needed a new kernel.**

   * **"existing `gemv_i8`"** — wrong by dtype. The artifact is bf16-verbatim, so
     `lm_head.weight` is bf16 and the kernel is `gemm_bf16`, the same one the MLP uses.
   * **"check `ARGMAX_BYTES`"** — `ARGMAX_BYTES` is `MAXROW * 8 + 4` = **20 bytes**, the argmax's
     (index, value) output plus a shared non-finite tag. The 808 KB/row this item means is the
     LOGIT buffer, `202048 * 4` = 808,192 B — a different allocation. Both are fine; conflating
     them writes the check against the one that was never at risk.
   * **the softcap had NO kernel.** `tanh` appeared nowhere in `kernels/`. `logit_softcap` now
     does `x = cap * tanh(x * mult / cap)`, with both constants as PARAMETERS — hard-coding either
     would put this model's numbers in a kernel the next model reuses.

   | | |
   |---|---:|
   | softcap vs host `tanh`, scale 400 (126,208 of 202,048 logits past 0.9·cap) | 9.54e-8 |
   | softcap vs host `tanh`, scale 1 (0 saturated — the linear region) | 7.60e-8 |
   | argmax over the full vocabulary, planted maximum at 202,045 | found |
   | head projection: all 202,048 outputs written, none left at 0 | verified |

   **The measurement changed this item's gate, and this is the finding.** `softcap_off` moves the
   `logits` bucket by only **4.879e-5 — 13.9× the fp32 floor**, against the 297× a `Rel` policy
   needs, so `tolerance::GLIMMER` carries `logits` as **`ExactOnly`**. That is not the instrument
   failing: at untrained weights the logits sit in `tanh`'s LINEAR REGION, where the softcap is
   nearly the identity. **The anchor cannot price this operator at all**, and S4 is where trained
   logits can. Meanwhile `ids` moves by exactly **0.000e0** at both draws — §5's argmax-invariance
   stated as a measurement rather than an argument.

   So the fixture scores against a host `tanh` at scale 400, where the function has saturated, and
   asserts a floor on how many logits pass 0.9·cap — feeding small values would reproduce the
   anchor's own blindness inside a test that then reports success.

   **Still owed, and it is §G3's:** nothing here shows the softcap is PRESENT in a decode. It
   cannot: that is what argmax-invariance means. G3 must carry a probability-space check.

   > **Review round 2 (2026-08-12, items 3–5 + the shared fixture), triaged and applied the
   > same day.** The two findings that change S3's contract, recorded here because their fixes
   > live in the kernels:
   >
   > * **`logit_softcap` was an Inf-launderer.** `tanhf(±Inf)` is ±1, so the plain form maps an
   >   overflowed logit to exactly ±cap — FINITE — one launch before `argmax`'s non-finite
   >   bail, which is the engine's ONLY detector for a fault after the last layer. The kernel
   >   now passes non-finites through unchanged, and the fixture proves ±Inf/NaN survive.
   >   `sigmoid_gate`'s analogue (a ±Inf gate becomes the limit, exactly 1 or 0) was reviewed
   >   and KEPT — the limits are what sigmoid converges to and NaN still propagates — recorded
   >   on the kernel as a decision.
   > * **Both new launchers hardcoded the null stream** in a tree whose streams are
   >   `hipStreamNonBlocking` — between two stream-ordered launches that is an unordered read,
   >   and no S2 fixture can see it (each launches one kernel and joins). Both now take the
   >   trailing `stream` of the `rivoli_swiglu` contract, WHILE the caller count is zero.
   >   **S3 must pass its stream, not null**, at both call sites.
   >
   > The rest: the launcher guards gained their driven table (9 rows incl. a TRANSPOSED
   > `(mult, cap)` refused as its own code — the swap is a silent sign-quantiser every greedy
   > gate passes); `output_multiplier` was the one logit-path constant with NO value assertion
   > anywhere (three uncrosschecked copies; now pinned to the vendored config); the trap-4
   > operand-separation guard compared RAW tensors where detectability rides on the SIGMOIDS
   > (|g−a| = 0.43 can coexist with a sigmoid signal of 8.3e-7, four decades under the bar —
   > now measured in sigmoid space, closest 2.92e-1); `worst_rel` silently discarded NaN
   > differences (`f32::max` ignores NaN — the 2026-08-05 trap, reintroduced; an all-NaN
   > kernel output scored 0.0, a perfect match); and the same insertion mistake orphaned doc
   > comments in THREE files (`hip.rs` triple-`# Safety`, `fwd.hip`'s argmax comment onto the
   > softcap, `kernel_coverage`'s gate comment onto the softcap row) — none of which any gate
   > can see, because `hip.rs`'s launcher region is jscpd-exempt (the ABI wall), `kernels/` is
   > unscanned, and clippy only checks a `# Safety` heading EXISTS. Inside those blind spots,
   > review is the only gate; the incident is recorded at the `hip.rs` marker.
