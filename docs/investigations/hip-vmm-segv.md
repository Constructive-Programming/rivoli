...
> | two-thread synthetic (`tests/vmm_threads.rs`), ~500 cycles/test | 2 in 8, then 3 in 12 | **0 in 20** |
>
...
> **THE CYCLE COUNT IS PART OF THE EVIDENCE, and the banner table did not carry it until
> 2026-08-15.** Review pointed out that the 7.14 column had been run with `tests/vmm_threads.rs` as
> it SHIPS — 6 cycles per test — while the 7.2 column beside it was row 3 of this bisect at ~500.
> That is 240 engine cycles of exposure against ~20,000, an 83x difference in the thing under test,
> and at the 7.2 per-cycle rate a clean 0-in-20 at 12 cycles/run would have had a **0.94**
> probability of happening even with the bug entirely unfixed. It looked like a refutation and was
> arithmetically almost nothing.
>
> **Re-run at 500 cycles per test, 20 runs: still 0.** Two observations fix the shape of the
> re-run, so it cannot be the same mistake twice: one run takes 1.99 s against the shipped 6-cycle
> file's 0.17 s, and both are predicted by a single (148 ms fixed, 1.84 ms/cycle) model — the fixed
> half being two fixture conversions. At the 7.2 rate (5 crashes in 20 runs of ~1000 cycles,
> p_cycle ~ 2.5e-4) the probability of 0 in 20 runs of 1000 cycles is **~0.007**. That is the number
> the closure rests on; the earlier one was 0.94.
...
**A dedicated VMM thread — HALF a fix, and the surviving half refutes the hypothesis.** It
marshalled every `rivoli_vmm_alloc`/`rivoli_vmm_free` onto one long-lived thread; it was
`memory::device`'s `vmm_thread`, and it was **deleted in `3cfbc1b`** once 7.14 made it
unnecessary, so nothing under that name exists to read. The synthetic two-thread gate went
**25% → 0 in 15**, which at that rate is
