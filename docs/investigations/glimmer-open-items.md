...
## 4b. OPEN — `glimmer_reference` segfaults, ~2 runs in 27, cause UNKNOWN

**Found 2026-08-14 by a full-suite sweep, not by any targeted work.** `cargo test --test
glimmer_reference -- --test-threads=1` died with **SIGSEGV** in
`the_engine_reproduces_muse_glimmers_own_logits`, after the file's other test had already passed
in the same process.

| how it was run | crashes |
|---|---|
| both tests, one process (first batch) | 1 in 5 |
| `the_engine_...` alone | 0 in 10 |
| both tests, one process (second batch) | **0 in 10** |

**Two observations in ~27 runs, so roughly 7%, and NOTHING is established about the trigger.**

> **A conclusion was drawn here and then withdrawn the same hour, which is why the table is
> above the prose.** After the middle row this file said the trigger was "the FIRST test having
> run first", on 1-in-5 against 0-in-10. The third row is the SAME configuration as the first and
> came back 0 in 10 — so the two configurations are indistinguishable at this sample size and the
> discriminator was noise. Rates that low need tens of runs per arm before they separate, and the
> first comparison had five.

**It is not GPU contention**, and that much the witness does settle: the crashing run had 1 KFD
holder and 18718720 B GTT — *identical* to three passing runs in the same batch.

**Not caused by that day's probability-space additions.** Those are host arithmetic over a
61-element `Vec<f32>` with no device call. Whether it predates 2026-08-14 is unknown.

**Why it matters beyond flakiness:** this is the gate that scores rivoli against Muse Glimmer's
own logits. A suite that fails a few runs in a hundred is one a future round will re-run until
green — which is how a real regression gets attributed to a flake.

Next step is a backtrace. `ptrace_scope` blocks gdb and eu-stack here (see the
`gpustream-test-hang` note), so the available instruments are a core pattern and
`/proc/<pid>/task/*/{stat,wchan}`.
