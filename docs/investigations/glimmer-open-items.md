...
> **DONE 2026-08-15, and this paragraph was wrong about all three of its predictions.**
> `Glimmer::layer` takes a row count and `prefill` calls it once per layer per chunk.
>
> **(a) The arithmetic did NOT change and no tolerance was re-derived.** Every projection is
> pointwise in position and both RMSNorms are one block per row, so `rows = R` is bit-identical to
> R calls at `rows = 1` — gated as `the_rmsnorm_row_dimension_is_a_reorder` and red-proved. The
> chain and reference gates pass unchanged, which is what says the restructure is a reorder.
>
> **(b) The ring hazard IS real and this file named the wrong mechanism.** It was reached with the
> attends still at `tq == 1`, so `rivoli_gqa_attend`'s guard never fired. What batched was the
> WRITES: a chunk writes every K and V before any of them attends, so at `ring_cap == win` writing
> position `p+1` overwrites the key at `p+1-win`, the oldest one still inside `p`'s window.
> `glimmer_chain`'s host reference caught it at **1.079e0**. The rule is about how many positions
> are LIVE IN THE RING at once — the chunk, whatever `tq` says — and the ring is now
> `win + PREFILL_CHUNK - 1`. There was a second defect the same shape: `Glimmer::branch` read row 0
> of `br`, which used to BE the last position, and scored **3.008e0** against the last position's
> reference.
>
> **(c) And it is NOT the highest-value S5 item, because `m` was never the lever.** Measured on the
> shipped bf16 model over 1659 prompt tokens: **0.363 s/token against 0.416**, ~13%, which is launch
> overhead. `kernels/kvcompress.hip::gemm_bf16` is ONE WAVE PER OUTPUT ELEMENT — `gw = r*n + c`,
> each wave reading the whole weight row `w + c*k` independently for every `r` — so it is `m` GEMVs
> fused into one launch and there is no cross-row reuse to capture. **The lever is a TILED GEMM**
> that stages a weight tile in LDS and reuses it down the rows; the batching is its precondition.
> The estimate here was made from the arithmetic without reading the kernel it depended on.
>
> **`gemm_bf16_tiled` landed the same day and is the other 1.74×** — one block per (output column,
> tile of 8 rows), the weight column staged in LDS and multiplied into all 8. Prefill is
> **0.204 s/token**, 2.04× the original. Dispatch is `m >= 8`, so decode keeps the wave-per-element
> kernel and its exact reduction order; the tiled form reduces differently, which is a different f32
> answer and the reason it is faster. **1.74 of a theoretical 8 says the bound MOVED rather than
> went**: the attends and RoPE are still per row, and `gemv_fp8` still refuses any `nrow` but 1 or
> 2, so the fp8 artifact gets almost none of this. Those are the next levers — and after today they
> get priced against a profile, not against arithmetic.
...
> **Item 4 — CLOSED 2026-08-15. The first quantized format is fp8, and the ladder has two rungs.**
>
> `convert_glimmer --fp8` (`artifact::format::add_quantized_fp8` over `quant::quantize_fp8_block`)
> writes **30.555 GB** against bf16's 55.712: the **416** layer projections as e4m3 at `FP8_BLOCK`
> plus their scale grids, `embed_tokens` / `lm_head` / the 209 norms untouched. The artifact came
> out 30,554,903,564 B against `layer_bytes(Fp8)`'s prediction of 30,554,760,704 — the 142,860-byte
> difference is the safetensors header, so the byte arithmetic the tier is sized from is confirmed
> against the file rather than asserted.
>
> | | PPL | mean dNLL | 95% CI (nats) | |
> |---|---:|---:|---|---|
> | bf16, 55.712 GB | 7.008490 | — | — | baseline |
> | **fp8, 30.555 GB** | 7.006667 | **−0.00026** | [−0.00701, **+0.00649**] | **PASS, conclusive** |
>
> **G4's wording is "does not straddle zero (or is recorded inconclusive)", and this is neither
> evasion.** The CI does straddle zero — but its upper bound is BELOW `bin/ppl`'s 1% bar (0.00995),
> which is that tool's own test for a null with power. `worse%` is 53.9. So the honest statement is
> not "fp8 is as good", it is **"any harm fp8 does is smaller than the 1% question this corpus can
> resolve"**, and that is a pass on G4's terms rather than the underpowered null the tool prints a
> paragraph warning about.
>
> **The plan's premise for preferring int4 is not refuted, it is deferred.** int4+g128's 13.65
> GB/token still puts the whole model under 14 GB where fp8 cannot, and that argument stands
> untouched. What changed is which rung was cheap to build: `quantize_fp8_block`, `place_fp8` and
> `gemv_fp8` all pre-existed, so fp8 cost a converter pass and a `GlimmerProj` sum type, and every
> line of that plumbing is what int4 will reuse. **int4 is the next rung and now needs only its
> arithmetic.**
>
> **What was NOT measured, and it is the interesting half.** The fp8 wall clock (141.9 s against
> 306.4 s) and a 220-token fp8 decode at 4.714 tok/s both ran with a NON-EMPTY witness —
> `/tmp/hiptest`, root, not ours, resolved after the fact — so both arms are discarded as speed
> measurements under this repo's own rule. Contention only slows an arm, so they read as lower
> bounds and fp8 is very probably ~2× on decode; **a clean format A/B is S5's and is not claimed.**
>
> **One free result worth more than it cost.** The bf16 arm was re-run only because the recorded
> `.nll` predated the ROCm 7.14 upgrade and pairing across a runtime change is a confound. The two
> runs are **byte-identical** — 0 of 761 per-token NLLs differ — so that confound is measured at
> exactly zero, and §4b's upgrade is now known to have changed no numerics on this path.

**S4 is CLOSED and G4 is met**: the checkpoint round-trips bit-exact where verbatim (item 1), the
template pin holds byte-for-byte (item 2), a human has read the output on both formats (item 3),
`tie_word_embeddings: false` is gated on addresses (item 5), and every format in the ladder has a
paired-dNLL row (item 4). **S5 is no longer gated.**
...
## 4b. SIGSEGV inside HIP's VMM path — **CLOSED 2026-08-14, upstream**

An **upstream** defect, fixed by **ROCm 7.14.0**. Re-run with rivoli's workaround BYPASSED:
`glimmer_reference` **0 in 20** (was ~3 in 52) and the two-thread synthetic **0 in 20** at ~500
cycles per test, the count its 7.2 column used (was 2
in 8, then 3 in 12). Nothing to file.

The dedicated VMM thread that had fixed the synthetic on 7.2 is **removed** — 103 lines in every
model's allocator, defending against a bug the runtime no longer has. `tests/vmm_threads.rs`
stays as the regression gate, because the shape it exercises (two `#[test]`s, two threads, one
binary) is what found this.

Full record, including the toolchain repair that was most of the work:
`docs/investigations/hip-vmm-segv.md`.

---

## 4c. `RoutedPool` unmaps its slab before joining the thread that writes into it — **OPEN, `scope: glm`/`v4`**

Reported by review 2026-08-14 while checking the `device_sync` that survived §4b. **Static
argument only; not reproduced.** Not Glimmer's — Glimmer has no `RoutedPool` — but it is recorded
here because it is what bounds the scope of that `device_sync`, and the comment on
`Drop for VmmBuf` now points at this entry rather than claiming a completeness it does not have.

`RoutedPool` has no `Drop` impl, so its fields drop in declaration order: `buf: VmmBuf`
(`routed.rs:237`) goes **before** `fetch: AsyncFetch` (`routed.rs:279`). So the slab is unmapped
first, and only afterwards does `AsyncFetch::drop` close its channel and join the reaper — which
drains any queued job on the way out (`for job in rx` yields buffered messages after disconnect)
and copies into `RoutedPool::host_ptr()`, an address inside the VA that was just released.

**`hipDeviceSynchronize` cannot close this in either direction**, which is the part worth keeping:
it does not wait on io_uring completions, and it does not stop a second thread from enqueueing
after it returns. Only an ordering — join the reaper before dropping the buffer, i.e. a `Drop` impl
or a field reorder — would.

Reachability is the unverified half. `AsyncFetch::submit` is fire-and-forget over an unbounded
channel and the decode thread's awaits are DEVICE-side, so the reviewer's path is an error return
between a submit and the end-of-layer join, followed by dropping the engine. On the success path
the end-of-layer `device_sync` drains it. **What to do first is not a fix but a repro**: no test
drops a `RoutedPool` with an outstanding `ReapJob`, so there is nothing today that distinguishes
"the reaper is always drained first" from "we have never happened to lose that race."

---

## 4d. The batched prefill's three open gaps — **OPEN, `scope: glimmer`**

Correctness review of `d1803de..1934045`, 2026-08-15. Six findings were fixed in that round's
triage; these three are recorded rather than fixed, because each needs a test run rather than an
edit and a half-built gate is worse than a written-down gap.

**(a) THE RING RESIZE DELETED ITS OWN REGRESSION GATE, and this is the sharpest of the three.**
`ring = (win + PREFILL_CHUNK - 1).min(n_ctx)` means a slot differs from its position only when
`n_ctx > win + 255`. The toy fixture's `sliding_window` is 2 and every engine-level gate builds
with `n_ctx` around 10, so `ring == n_ctx` and `slot == pos` — **the identity map**. Before the
resize those runs had `ring_cap == 2` and wrapped on every other position, scored per position
against a host reference. So the 1.079e0 defect that resize FIXED now has no numeric gate: the only
run that still wraps is `the_prefill_is_layer_major_and_the_fill_count_says_so` (300 tokens), and it
asserts fill counts, never output. Worse, `glimmer_chain.rs` and `glimmer_loop.rs` still assert
`fed.len() > 2 * sliding_window` under the message "the run must wrap the ring" — **true and
vacuous**, and their docs still say the ring wraps four times.
What closes it: one run with `n_ctx > sliding_window + PREFILL_CHUNK - 1` scored against a
reference. The cheapest honest reference is the same engine fed one token at a time, which takes
`rows = 1` everywhere; a too-small ring makes the two disagree.

**(b) `polluted_rows`' reference and subject now go through DIFFERENT KERNELS.** It builds `want`
with `gemm(1)` and its subject with `gemm(FENCE_ROWS)` where `FENCE_ROWS` is 4096, and compares with
an exact slice `!=`. Since `1934045` those are the wave-per-element and tiled kernels, with
different reduction orders. It passes today — at `FENCE_DIM` 256 the fixture's bf16 values sum
exactly in either association — so this is a fragility rather than a live failure, and the failure
mode is bad: arm B would report 4096 of 4096 rows corrupted and blame `GlimmerPin::layer`'s fence,
while arm A's `diverged > 0` would go vacuously true and retire the red proof. Fix by taking the
reference through the same kernel, never by loosening the comparison — the gate needs bit-identity
to tell a stale read from rounding.

**(c) `rivoli_gemm_bf16` is shared with DeepSeek-V4's KV compressor, and its prefill silently
changed reduction order.** `kvcompress.rs` calls it with `m = seqlen`, always `>= 8` in practice, so
every V4 compressor prefill now takes the tiled kernel. The dispatch note argues the threshold
purely in terms of Muse Glimmer decode and nothing records that a second model's numerics moved.
No evidence it breaks that path's oracle tolerances — both forms re-associate the same dot — but an
unannounced numeric change outside a diff's declared scope is what `scope:` exists to prevent.
Either price it on V4's own gates or make the threshold a caller's choice rather than `m` alone.

Also recorded from the same review, not yet acted on: the tiled kernel is verified only at toy
extents (`K` 1100, `N` 24, `m <= 19` against shipped `k`/`n` up to 19968 and `m` 256); nothing gates
the dispatch THRESHOLD itself, so "decode keeps the old kernel bit-identically" is asserted and
untested; and `acc[GEMM_TILE]` is indexed by a runtime-bounded loop, so if the compiler declines to
unroll it the array lands in scratch and the reuse evaporates — worth one ISA dump before trusting
the 2.04x.
