...
> * **`tests/glimmer_loop.rs` gates the two selections deviceless** (the window and the QK scales)
>   and `kernel_coverage.rs`'s OWNERS census now attributes eleven launchers to `glimmer_gpu.rs`.
>   That closes **trap 2's wiring half** — a port that never calls `rmsnorm_weightless_batch` at
>   all passes every assertion in `glimmer_qk_norm.rs`, and the census is what sees it — and
>   NARROWS **trap 3** without closing it: the scales arrive by name, so the swap is a rename
>   rather than an argument reorder, but a call site reading `s.q` twice still passes.
> * **What this does NOT do is prove the loop is numerically right, and nothing in the tree can
>   yet.** G3's zero-tolerance comparison needs the tiny model's WEIGHTS, and the vendored anchor
>   goldens hold activations only — 1,099 tensors of captured intermediates across 4 layers and 7
>   steps, and not one parameter. So the options are: vendor the tiny checkpoint, or drive the
>   loop from captured activations the way the S2 kernel gates already do. **That choice is G3's
>   first decision and it is not made here.**
