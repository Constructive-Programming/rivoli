...
> > reference transcribed from §3-§5 and scored against the engine's logits at **2.3e-6** over a
> > 9-position run that wraps the sliding ring four times. Six of the seven mutations review
> > enumerated are red-proved on it (k/v swap 1.5e0, `qk_scale` dropped 1.7e0, q/gate swap 9.6e-1,
> > NoPE inverted 7.9e-1, split-half→interleaved rope 8.5e-1, `l % 4 == 3` at 9.9e-1), and the
> > seventh is the finding —
> >
> > **THE QK SCALE SWAP IS AN IDENTITY.** Both operands are weightless-normed before the scale, so
> > the score carries only their product, and RoPE commutes with a scalar. Handing 3.87 to K instead
> > of Q cannot change any output, at any position, including against cached keys. This tree called
> > it "fluent and wrong" in four places and gated it as a hazard; §9 trap 3 is corrected in place,
> > `QkScale` records the measurement, and what is asserted now is the PRODUCT. Dropping the factor
> > is the live mistake and is the 1.7e0 row above.
> >
> > **`l % 4 == 3` reddens ONLY on a rotated `layer_types`**, and that is not a weakness of the
> > gate: any prefix of this checkpoint's own `[s,s,s,full]` period agrees with the modulo, so no
> > test using the shipped pattern can tell them apart — which is exactly what that field's doc
> > says. The permuted-config test is what closes it.
> >
> > **Two mutations stay invisible and are recorded rather than papered over.** `attn_scale` from
> > `hidden` instead of `head_dim` is a no-op on a fixture whose `head_dim` EQUALS its width — it
> > preserves `head_dim != hidden / n_heads` (trap 15) but not `head_dim != hidden`, and closing
> > that touches every shape it writes. And an `eps_post`/`eps_pre` swap on a branch norm moves the
> > norm factor by ~5e-6 where `mean(x²)` is O(1), below this file's tolerance — gated instead by
> > `glimmer_head.rs`'s 41.8-56.6x eps census, where the statistic is small.
> >
