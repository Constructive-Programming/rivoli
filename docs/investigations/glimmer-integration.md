...
> **THE LOOP LANDED 2026-08-13.** `src/glimmer_gpu.rs`, and `run_glimmer`'s S1a bail is gone — a
> Glimmer artifact now tokenizes, prefills, decodes greedily and prints. Items 0-5 below were its
> preconditions and are all paid; what follows records what the loop itself decided, and what it
> still does not prove.
>
> * **The chain is §3 and §4 verbatim**, and each trap is handled where no call site can restate
>   it: `pre_norm`/`branch_add` take their eps from the half they belong to (so the 1e-5/1e-8 swap
>   is not expressible at the level that picks which norm runs where), `branch_add` norms the
>   BRANCH and then adds, `attention` builds the gate from the layer input, and the QK scales
>   arrive as a named `QkScale` pair.
> * **`window_of` is the answer to the truncation trap**, and the fix is a shape rather than a
>   check: one `match` on `layer_types` yields `win`, `ring_cap` and the slot together, so a full
>   layer cannot be handed `win != 0` with `ring_cap == 0` — the combination the launcher ACCEPTS
>   and which silently bounds a global layer to its last 2048 rows. `Glimmer::new` sizes each
>   layer's cache from that same function, so the ring the loop indexes and the buffer it was
>   allocated cannot disagree.
> * **The whole loop is on the NULL stream, uniformly, and that is item 4's contract met rather
>   than dodged.** Six of the launchers the chain needs take no stream parameter at all
>   (`rope_split_half`, `rmsnorm_single`, `vadd`, `argmax`, `rope_interleave`, `swiglu`'s
>   siblings), so a "compute stream" loop would be exactly the inversion item 4 warns about — a
>   real stream at the launches that accept one and the legacy default at the six that cannot.
>   S5's change is giving those six a stream, not passing one here.
> * **`tests/glimmer_loop.rs` gates the two selections deviceless AND runs the loop** on the toy
>   checkpoint, and `kernel_coverage.rs`'s OWNERS census attributes twelve launchers to
>   `glimmer_gpu.rs`. That NARROWS **trap 3** — the scales arrive by name, so the swap is a rename
>   rather than an argument reorder, though a call site reading `s.q` twice still passes.
>
> > **THE LANDING COMMIT'S OWN CLAIMS DID NOT SURVIVE ITS REVIEW ROUND, and two of them were
> > load-bearing.** Recorded here rather than rewritten above, on this file's own convention.
> >
> > **The loop could not decode.** `window_of` clamped a sliding layer's ring to `n_ctx` and left
> > `win` at `sliding_window`; `attn.hip:647` rejects `ring_cap < win + tq - 1` with code 1005, so
> > with `--bench` defaulting to 64 the default invocation refused at layer 0 — and the deviceless
> > gate that shipped with it asserted exactly that pair as correct. Both reviews found it
> > independently. The clamp now takes `win` with the ring, which is the same attention (a context
> > shorter than the window makes every position's causal bound 0 either way), and the gate asserts
> > the LAUNCHER's inequality across a sweep of `n_ctx` instead of a literal triple.
> >
> > **"Closes trap 2's wiring half" was wrong.** The census tests whether a FILE contains a line
> > naming the launcher — not how many call sites, not whether any runs. Delete both QK-norm calls
> > and the embedding norm's call keeps the row green. Retracted; what closes it is a test that
> > runs the loop.
> >
> > **The premise that no test could run the loop was also wrong**, and it is what let the 1005 ship:
> > "needs 55.7 GB of weights" is false for the toy fixture, which converts and pins exactly like the
> > real checkpoint. `tests/glimmer_loop.rs` now decodes on it. **But a fixture only reaches defects
> > on axes it spans, and that is measured, not assumed:** with the broken window restored the
> > fixture decode stayed GREEN — its `sliding_window` is 2, so its ring is never short — while the
> > `n_ctx` sweep went red.
> >
> > Five more, each fixed: `sample()`'s finiteness check read `argmax`'s INDEX, which the kernel
> > never makes negative, rather than its VALUE — deleting the only post-final-layer fault detector
> > one launch after `logit_softcap` deliberately preserves it; `decode` pushed the stop token into
> > its output where `GpuEngine::generate` does not; the layer body captured pin pointers across two
> > `pin.layer` calls, which `GlimmerPin::layer`'s contract forbids in as many words and which was
> > safe only because all three requests named the same layer (resolved once into a `Handles` now,
> > which also un-inflates the pin's hit counter threefold); the null-stream justification miscounted
> > its own inventory — four launchers take no stream, not six, and it listed `rope_interleave`, the
> > convention this model must never use; and no error path joined the device before `Glimmer`
> > dropped, which unmaps the weight slab under live kernels.
> >
> > **The consumption gate that round asked for is BUILT** — `tests/glimmer_chain.rs`, a host
> > reference transcribed from §3-§5, scored against the engine's logits **per position** (nine of
> > them, each against a freshly built engine, so a red names where divergence starts). Worst green
> > position **3.9e-6**. Nine wiring mutations red-proved: softcap deleted 4.1e0,
> > `output_multiplier` dropped 4.1e0, split-half→interleaved rope 1.8e0, q/gate swap 1.3e0, k/v
> > swap 9.0e-1, NoPE inverted 3.2e-1, `l % 4 == 3` at 2.4e-1, `qk_scale` dropped 1.1e-1, and the
> > softcap's `tanh` alone at 9.9e-5.
> >
> > **THE TOLERANCE WAS ABOVE A REAL DEFECT AND THE GATE PASSED IT.** The file shipped at `TOL =
> > 1e-4` justified as reduction noise; the `tanh`-only mutation — the argmax-invariant failure
> > `Glimmer::logits` was added to make visible — scored 9.9e-5 and PASSED one of the two tests by
> > 1%. The tree had already measured that class at 4.879e-5 on the anchor, so the margin was
> > knowable. `TOL` is now 2e-5, set from the weakest red-proved mutation. The "differs only in
> > reduction order" justification was independently wrong on three counts: an online softmax
> > against a two-pass one, FMA contraction under `-ffp-contract=fast`, and device against host
> > libm.
> >
> > **The QK scale swap: the claim was right and the framing was overstated.** It does not change
> > the LOGITS — both operands are normed before the scale, so only the product enters, and RoPE
> > commutes with a scalar. But it is an identity in EXACT arithmetic only, is not byte-identical,
> > holds only while the KV cache is f32 (a reduced-precision K cache is an S5 lever), and **it does
> > change `q` and `k` themselves**, which the anchor captures elementwise — a swapped engine is
> > 3.87x off at S4's tensor-vs-capture scoring. §9 trap 3's own clause, "to K **as well as** Q", was
> > never about the swap, is untouched, and is red-proved by the dropped-factor row. And the algebra
> > was already recorded in `tolerance.rs` on 2026-08-12; only the doc's fluent-and-wrong framing
> > was new.
> >
> > **THE EPS ASSIGNMENT IS OPEN, and was filed as covered by the wrong file.** Swapping `eps_post`
> > and `eps_pre` at `pre_norm`/`branch_add` is below this gate's tolerance, and the citation to
> > "`glimmer_head.rs`'s eps census" was wrong twice: that file has no eps census (the 41.8-56.6x
> > figures are `glimmer_norm.rs`'s), and what those gate is the OPERATOR's eps sensitivity, not
> > which eps the loop hands it — `glimmer_norm.rs` never imports `glimmer_gpu`. Swap the two call
> > sites and nothing in the tree reddens.
> >
> > **Two more blind spots, recorded rather than papered over.** `attn_scale` from `hidden` instead
> > of `head_dim` is a no-op on a fixture whose `head_dim` EQUALS its width. And trap 10's KV-head
> > broadcast is unconstructible with one KV head — `head/(hq/hkv)` and `head % hkv` are both 0 —
> > so the oracle carries that expression as decoration; it is gated at kernel level by
> > `glimmer_attend.rs`.
> >
> > **`l % 4 == 3` was re-measured with the allocation mutated in lockstep**, because review argued
> > the first proof reddened through a device write past a cache sized from the true kinds. Same
> > 2.353e-1, so it is the masking difference and not an overrun.
> >
> > **Still open, not fixed:** the KV cache is `hipMalloc`'d outside the budget the operator was
> > shown (up to 3.4 GB at the 131072 ceiling, against a 4 GiB driver headroom).
>
> * **What this does NOT do is prove the loop is numerically right, and nothing in the tree can
>   yet.** G3's zero-tolerance comparison needs WEIGHTS, and the vendored anchor goldens hold
>   activations only — 1,099 tensors of captured intermediates across 4 layers and 7 steps, and not
>   one parameter. So the options are: vendor the tiny checkpoint, drive the loop from captured
>   activations the way the S2 kernel gates already do, or score it against a host oracle over the
>   toy fixture's synthetic weights. **That choice is G3's first decision and it is not made here.**
> * **Prefill is token-major** (`tq == 1` throughout), which is what lets every sliding ring be
>   exactly `sliding_window`. It is also the slow way and it re-streams the whole model per prompt
>   token at any budget below full residency — S5's, and marked `ponytail:` at the call.
> * Two `glimmer_flags.rs` tests were keyed on the deleted bail's SENTENCE rather than on their
>   subject, and went red. Re-keyed onto the partition report, which survives the loop landing.
...
4. **Streams, not null.** `sigmoid_gate` and `logit_softcap` take the trailing stream now;
   ...
   > **PAID 2026-08-13, and the item was one-third done, one-third unbuildable, and one-third
   > wrong.** `tests/glimmer_stream_order.rs`.
   >
   > * **Already true**: both launchers took the trailing stream at S2. Nothing to do.
   > * **Unbuildable**: "the loop passes its compute stream at both call sites" needs the loop.
   >   Neither launcher has a `src/` caller — `kernel_coverage.rs`'s OWNERS rows are empty and
   >   that census fires the moment one appears. Same shape as item 3: a contract, not a wiring
   >   gate.
   > * **Wrong, and this is the finding: "no fixture can see it" is false.** The FIRST clause
   >   is confirmed — a null stream there really is an unordered read. Only the second fell. A
   >   **3.48–3.86 ms** `gemm_bf16` on a real stream writes the consumer's operand; the consumer
   >   is enqueued **5.7–34.5 µs** later, **108–656× inside** the producer.
   >
   > | consumer's stream | elements disagreeing with the ordered answer, of 2,097,152 |
   > |---|---:|
   > | **null** | **2,095,258 – 2,097,152 (99.910–100.000%)**, 14 measurements, both launchers |
   > | **the producer's** | **0**, every run |
   >
   > Red-proved by dropping the stream argument in `kernels/fwd.hip`
   > (`(hipStream_t)stream` → `(hipStream_t)0`, run and reverted): the green column goes to
   > 2,097,152 and 2,095,292, so it is the stream being honoured and not the race failing to
   > fire. The 0.09% that is not stale is `gemm_bf16`'s earliest blocks retiring in
   > microseconds — which is why the floor is `> OUT/2` and not a fraction.
   >
   > **The two launchers fail in DIFFERENT ways, and that changes what the softcap's call site
   > is risking.** `sigmoid_gate` reads one buffer and writes another, so every wrong element is
   > a stale read — **0 lost writes, every run, asserted rather than observed**. `logit_softcap`
   > read-modify-writes the head's output, and there the split inverts: **0–261 elements kept
   > the capped stale value; 2,095,254–2,097,152 lost the consumer's write entirely.** So a null
   > stream at the head does not compute a *wrong* softcap. It computes the right one and lets
   > the raw logits overwrite it — **the softcap simply does not happen**, which is precisely
   > the failure §9 trap 12 says no greedy gate can see. The first version of the gate asserted
   > "the surviving value is the uncapped logit" in prose and measured nothing.
   >
   > **The contract as this item states it is too weak, and must not be carried into the loop
   > as written.** "Passes its compute stream at both call sites" is satisfied by a compute
   > stream at exactly these two launches inside an otherwise null-stream layer — the same bug
   > inverted. What has to hold is that every launch touching one buffer is on one stream, or
   > separated by an explicit event or sync. **The FULL invariant needs interprocedural
   > dataflow** — which buffer a launcher touches, and where the syncs land — so it does not
   > mechanise. `src/f4gpu.rs` shows both halves of why: `pre_norm` and `layer` hand
   > `launch_hc_pre` / `launch_hc_post` a null *on purpose*, because those functions are
   > null-stream throughout; `routed_experts` mixes `compute_stream`, `miss_stream` and a null
   > in one body and leans on four explicit `device_sync` boundaries its header enumerates. Null
   > is correct in both, for reasons no grep can see. So the two OWNERS rows carry the price tag
   > instead of a rule.
   >
   > **OPEN, found by review and deliberately not done here — a NARROWER rule DOES mechanise.**
   > `src/backend.rs:86` defines `NULL_STREAM` precisely so a deliberate null reads as a
   > decision rather than a default, and **about fourteen `src/` call sites still pass a bare
   > `std::ptr::null_mut()` in the stream position** — including `src/f4gpu.rs:1686`, in the file
   > that imports `NULL_STREAM` and uses it eight lines away. A source census in
   > `kernel_coverage.rs`'s style would cost one test and no device. Left out of this item
   > because it edits `gpu.rs`, `f4gpu.rs` and `attn.rs`, three engine files this port does not
   > own and other worktrees are working in; it is the owner's call whether to spend it.
   >
   > **Still owed by the loop**: the call sites themselves, which only G3 can gate.
5. **NoPE pattern [L,L,L,G]**, window 2048 ring on sliding layers, **two EOS ids**
   ...
   > **PAID 2026-08-13. Three of the four clauses were already met by earlier stages, and the
   > fourth was live in a form one step worse than the plan describes.**
   >
   > * **NoPE pattern — DONE at S1a, nothing to add.** `layer_types` is `Vec<LayerKind>`, so an
   >   unknown spelling is refused by **serde** rather than by a `validate` a caller can skip,
   >   and `layer_rope_theta` is checked as its boolean twin: *sliding IFF rotated*, gated in
   >   BOTH directions by config-mutation tests, plus an unknown-kind case and a rotated layer
   >   asking for a base the one shared table is not built from. What is NOT gated is the loop
   >   calling `layer_kind(l)` instead of `i % 4 == 3` — there is no call site, so this is
   >   item 2's trap 3 one more time and it belongs to G3.
   > * **Window 2048 ring — the KERNEL and its launcher guard are DONE at S2 item 1; the
   >   PER-LAYER SELECTION has no call site and belongs to G3.** `gqa_attend` derives the causal
   >   bound from `(start_pos, win)` — `ring_cap` is the slot map and never an input to that
   >   bound, which is the one relationship the launcher guard exists to sever, and calling it a
   >   three-argument derivation (as this plan and item 1's record both did) points a reader at
   >   exactly the wrong coupling. 72 ring cases, and the launcher refuses `ring == win` for any
   >   `tq > 1` because a launch attends the union of its query rows' windows.
   >
   >   The clause the plan wrote is "window 2048 ring **on sliding layers**", and the per-layer
   >   part is the half that can be wrong: `launch_gqa_attend` has no `src/` caller, so nothing
   >   yet derives `win` from `layer_types`. Review named the live mistake it leaves open —
   >   **`win != 0` with `ring_cap == 0` on a `full_attention` layer is ACCEPTED** (the guard
   >   rejects only the inverse) and silently truncates the causal prefix to the last 2048 rows,
   >   fluent and wrong. Unreachable today; the first thing G3's loop can get wrong.
   > * **Softcap in the tail** — G3's probability-space check, which nothing at S2 could make.
   > * **The EOS clause, and here is the finding.** The engine half was already safe:
   >   `Tokenizer` holds `eos: Vec<u32>`, `load_eos` reads both the array and the bare-int
   >   spellings, and the stop test is `eos.contains(&t)` — plural throughout, shared with every
   >   model here. So "a scalar-EOS port stops on one" was **not** the live risk.
   >
   > **The live risk was that `REQUIRED_AUX` checks EXISTENCE and nothing checked CONTENT.** This
   > tree's own Glimmer fixture wrote `generation_config.json` as `{}` — which satisfies that
   > check, copies into the artifact, and gives the engine **zero** stop tokens. A port built on
   > it does not stop on one of the two; it stops on **none**, announced by a single `warn!` at
   > load. That signature is already in this repo's record: `docs/measurement/benchmarks.md`'s
   > retraction is 56 runs where not one terminated naturally and every one hit its token limit.
   >
   > `convert_glimmer::eos_ids` now parses the file before any tensor is read and refuses one
   > carrying no usable id, through the SAME two spellings `Tokenizer::load_eos` accepts — a gate
   > that read the file differently from the engine would certify something the engine then
   > rejects. Non-empty rather than "exactly two", on `layer_types`' own argument: the pair is a
   > fact about this checkpoint, not a rule about the architecture. Checked in the converter
   > rather than in `artifact/tokenizer.rs` because that path is shared with three other models.
   >
   > **AND THE FIRST VERSION CHECKED THE WRONG FILE.** It read the SOURCE, while the invariant —
   > "the artifact would ship with no stop token" — is a property of the ARTIFACT, and between
   > them sits the very defect `REQUIRED_AUX`'s own note was written for: `finish_artifact`
   > downgrades a failed aux copy to a warning and returns `Ok(())`. So the converter could still
   > exit 0 with an artifact carrying no `generation_config.json` at all — filesystem full after
   > the 53 GB write, mount gone read-only, source file removed during a three-hour run. The
   > check now runs **again against `out_dir` after `finish_artifact`**, and compares the ids to
   > the checkpoint's. Red-proved by making the copy skip that file: the converter fails where it
   > previously exited 0. Found by review 2026-08-13, against a version whose *test* already
   > asserted the artifact-side property while the shipped binary asserted only the source-side
   > one.
   >
   > **An id past the vocabulary is refused too**, which is the same unstoppable decode one layer
   > down: a stop token no argmax can return never fires. The check moved one line later so
   > `g.vocab` is available, still before any tensor is read.
   >
   > `both_eos_ids_reach_the_artifact_and_an_empty_generation_config_is_refused` writes a
   > DISTINCT pair before the happy path and compares the artifact's copy to the source's
   > **bytes** — against the fixture's own ids it would be satisfied by a converter emitting that
   > constant from anywhere, and a parsed-id compare would pass a reordering or an added key.
   > Five red arms, each proved red: `{}`, an empty array, a null, an out-of-vocab id, and the
   > skipped copy.
   >
   > **The fixture's ids now SCALE with its vocabulary**, and the first fix got that wrong too. It
   > wrote the real `[200001, 200008]` against a fixture `vocab_size` of **12** — shrinking one
   > half of the pair with the model and copying the other verbatim. The vocab bound above would
   > have gone red on it, making "weaken the check" the cheap repair: precisely the retreat this
   > file keeps warning about, set up by the same commit that warned about it.
   >
   > And `eos_ids` no longer re-implements the engine's parser. It was fifteen lines re-spelling
   > `Tokenizer::load_eos` with a comment asserting the two "match"; `artifact/tokenizer.rs` now
   > exports `eos_token_ids` and both callers use it, so that claim is structural instead of
   > asserted. The refusal stays in the converter — the shared path warns rather than failing,
   > which is three other models' call to keep.
   >
   > **This is the same lesson at the same file, twice.** 2026-08-11's review found the fixture
   > shipping two of four aux files, so every green run certified the shape where trap 13 lives.
   > The fix wrote all four as `{}` — the minimum the new check accepted — and thereby certified
   > a strictly worse shape for two more days. **A fixture written to the letter of a gate stops
   > testing the gate**; write what the checkpoint actually has.
