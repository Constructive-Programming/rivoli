...
5. **NoPE pattern [L,L,L,G]**, window 2048 ring on sliding layers, **two EOS ids**
   ...
   > * **Window 2048 ring — DONE at S2 item 1.** `gqa_attend` DERIVES the causal bound from
   >   `(start_pos, win, ring_cap)`, 72 ring cases, and after review the launcher refuses
   >   `ring == win` for any `tq > 1` because a launch attends the union of its query rows'
   >   windows. G3 owns the decode crossing 2048.
   ...
   >
   > `both_eos_ids_reach_the_artifact_and_an_empty_generation_config_is_refused` asserts the
   > artifact's copy parses back to `[200001, 200008]`, with three red arms — `{}`, an empty
   > array, a null — each proved red by disabling the check and each refusing before the artifact
   > exists. The fixture now writes the real pair.
   >
