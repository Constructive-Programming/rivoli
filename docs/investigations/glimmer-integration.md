...
4. **Streams, not null.** `sigmoid_gate` and `logit_softcap` take the trailing stream now;
   ...
   >   gate.
   > * **Wrong, and this is the finding: "no fixture can see it" is false.** A 3.8 ms
   >   `gemm_bf16` on a real stream writes the consumer's operand; the consumer is enqueued
   >   **6–42 µs later**, inside the producer's first 1%.
   >
   > | consumer's stream | elements disagreeing with the ordered answer, of 2,097,152 |
   > |---|---:|
   > | **null** | **2,095,272 – 2,097,152 (99.91–100.00%)**, 14 measurements, both launchers |
   ...
   > fire. The 0.08% that is not stale is `gemm_bf16`'s earliest blocks retiring in
   > microseconds — which is why the assert is `> 0` and not a fraction.
   >
   ...
   > separated by an explicit event or sync. **That is not statically checkable**, and
   > `src/f4gpu.rs` is the proof: it mixes four streams in one function and its header records
   > `launch_hc_pre`, `launch_hc_post` and `launch_moe_acc_drain` as taking a stream and being
   > handed null *on purpose*, "because everything around them is null-stream, so a non-null one
   > would reorder against the norms". Same premise, opposite conclusion. So there is nothing to
   > mechanise here beyond the census, and the two OWNERS rows now carry the price tag instead
   > of the rule.
   >
