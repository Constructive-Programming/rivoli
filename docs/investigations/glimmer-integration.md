...
> >
> > **Still open from that round, not fixed here:** the KV cache is `hipMalloc`'d outside the budget
> > the operator was shown (up to 3.4 GB at the 131072 ceiling, against a 4 GiB driver headroom), and
> > `tests/glimmer_loop.rs` gates the two helper functions plus execution — it cannot see a call site
> > that consumes them wrongly, so a `l % 4 == 3` kind selection, a Q/gate or K/V matrix swap, an
> > inverted NoPE guard, or `rope_split_half` → `rope_interleave` all still pass every gate in the
> > tree and all produce fluent wrong text.
>
