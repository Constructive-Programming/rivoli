...
**The break-even N was derived at fp8, and bf16 is what ships by default.** Reported to M17 by the
coordinator 2026-08-17, and it is the sharpest form of the point below: the plan's headline
"break-even N > 1.1 accepted tokens" — the number justifying this whole track against GLM's 53%
acceptance — **was derived at the fp8 target**, while the default-shipping target is bf16. A dense
verify pass reads every weight once regardless of row count, so a *cheaper* target makes the
drafter's fixed cost a *larger* fraction of the step and pushes break-even N **up**. So N > 1.1 is
the fp8 figure and the bf16 figure is larger by an unmeasured amount, in the direction that makes
DFlash harder to justify on the configuration that ships today.

**Nothing here measures either one**, and M17e must not quote 1.1 without naming the dtype it came
from — that is the same failure as the three per-token budgets above, one level up: a ratio derived
under one configuration and read as if it were the model's.
...
### DECIDED 2026-08-17: the export is bf16, and the ring is 260.0 MiB

The question above — narrow the export or pay 520.0 MiB — was posed as a quality-versus-bytes trade
and **that framing was wrong**, which is worth recording because it is what made the decision look
hard. The owner supplied the grounding that dissolves it.

**Argument 1, and it is the one that decides: bf16 is REFERENCE-FAITHFUL, not a narrowing.** Read
from the checkpoints on this box:

| file | `dtype` |
|---|---|
| `muse-glimmer-30b/config.json` (the target) | `bfloat16` |
| `muse-glimmer-30b-assistant/config.json` (the drafter) | `bfloat16` |
| `glimmer-30b-bf16/manifest.json` (the shipped target artifact) | `bfloat16` |
| every one of the drafter's 58 tensors | `BF16` — the dtype set is exactly `{BF16}` |

So in the reference stack the target runs bf16 and **the hidden states the drafter consumes are
bf16**. The drafter has never been trained or validated against more precision than that. An f32
export would hand rivoli's drafter *better* input than the model was built for, at double the cost —
which is not a conservative choice, it is an unvalidated one.

**Argument 2: P5.** Bytes per token are the currency and this is a straight halving on a
bandwidth-bound arm — 133,120 → 66,560 B/token, and the ctx-4096 ring 520.0 → 260.0 MiB.

**Argument 3, recorded WITH its correction, because it was offered as independent and it is the
weakest of the three.** The argument was: the drafter's first op multiplies by BF16 weights, so
mantissa beyond bf16 is discarded at the kernel boundary regardless. **That is not strictly true.**
An f32 activation against a bf16 weight yields a more accurate product than a bf16 activation
against the same weight — the weight's width does not truncate the activation's, and the
accumulator here is f32. So argument 3 does not close the gap on its own; the conclusion rests on 1
and 2, which are sufficient. Kept rather than dropped because the *shape* of it is right — the
precision has nowhere to go that the model was tuned to use — and because an argument accepted
without checking is how a wrong one survives to be quoted.

**The f32 column stays in the record with its derivation** (KV 40.0 KiB/token, export 133,120
B/token, 520.0 MiB ring) and both columns stay asserted in `drafter_convert.rs`. **The f32 path is
NOT built** — YAGNI, and there is no second consumer. If acceptance ever measures marginal against
break-even, an f32-export arm is the obvious A/B and its sizing is already here; that is the **named
fallback**, not code.

**One thing this decision does not settle**, so nobody reads it as settling it: the engine's KV cache
and residual stream remain **f32** (`glimmer::pin::scratch` is `DeviceBuf::new(n * 4)`,
`geometry::kv_bytes` ends in `checked_mul(4)`). Only the drafter's hidden-state EXPORT is bf16. The
drafter's own KV is a separate allocation and a separate question, and at f32 it is 40.0 KiB/token.
...
**Every gate in `crates/cli/tests/drafter_convert.rs` is proven red.** The battery, the coverage
matrix, the count and the plants worth reading are in **`docs/measurement/gate-red-proofs.md` §6**
— not repeated here, because a second copy of the same table is a thing to drift rather than a
check.

> **CORRECTED 2026-08-17: this paragraph had already drifted, twice, one line above the sentence
> explaining why copies drift.** It read "All twelve gates are proven red, by six plants" while the
> suite had grown to 14 and the battery to 12, and the front matter read a third, differently-stale
> pair ("THIRTEEN gates … nine plants"). `tests/docs.rs` compared the front matter against its
> INDEX row and they agreed — with each other, and with neither the tests nor the battery, because
> nothing gates a number in prose. **The counts are now deleted rather than corrected a third
> time**: they live once, in §6, next to the table they are derived from. A pointer cannot go
> stale; a transcribed count is a promise to keep transcribing it.
