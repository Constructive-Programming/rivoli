...
this is a regression gate, not a measurement — no tok/s is claimed.

**324 passed / 0 failed** across all 26 binaries at the **dev profile** (`--features rocm`,
`--test-threads=1`, each suite under the flock).

**Not comparable to the rename gate's "union suite 334/0" above, and the difference is method:**
that was `--release`, union features, and all 35 targets, which counts the 7 `src/bin/*`
targets a `--lib`/`--test` run never builds. No union all-targets run was completed here, so
334 is neither reproduced nor contradicted.

Every earlier attempt this session was discarded under a witnessed foreign tenant — 33.6–39.5
GiB of GTT with zero kfd entries and no flock (`reference/gpu-lock.md`), here llama-swap
serving qwen3.6-ultra at 262k ctx. Cleared by cordoning and draining `rh-anine`: GTT
39517 → **17 MiB**, kfd 0, lock free, and the witness was clean before AND after every suite.
