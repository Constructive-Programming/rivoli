...
>
> **The re-run is OWED, and so is the red proof.** No `bit_identical` value is claimed here until
> `--mode indexer-identity` has been run against the sentinel arm, and until a planted change to
> `indexer_variant_forward`'s `variant=None` body has been OBSERVED to make it print `False` — the
> old gate's defect was precisely that it printed `True` regardless, so a green from the new one
> means nothing until the red is witnessed.
