...
* **The n-gram gate against track A's derivation is OWED, and it is blocked on a BRANCH, not on a
  ...
  that matters, distinguishing this checkpoint's PLE layer index. **LANDED 2026-09-02** as
  `qwen_anchor_windows.rs::the_ports_ngram_derivation_reproduces_the_goldens_int64_buffers_at_ordinal_zero_only`,
  asserting exactly the five quantities above and red-proofed by handing the equality arm ordinal 1
  (`gate-red-proofs.md` §15). The second trial-division `is_prime` stays deliberately independent of A's, argued
