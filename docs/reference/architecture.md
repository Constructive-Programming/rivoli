...
| INV-9 | **Nothing on the host path can make two runs of one input a different program.** The routed path takes exactly one host decision that reads DEVICE PROGRESS rather than its own inputs — `AsyncFetch::take_slot`'s staging-slot hand-out, whose predicate is a timeline value, i.e. wall-clock. It is deterministic because of a barrier somewhere else: `glm::forward::run_layer` ends every layer with an unconditional `device_sync`, so at the next `submit` every prior bounce copy has retired, the predicate is uniformly true, and `scan_free` is pure round-robin over the miss sequence. `AsyncFetch::slot_stalls()` is the observable that falsifies the precondition — non-zero means it did not hold and a determinism comparison over that run is unsound. **Scope, stated because it is narrower than it reads:** this is the same-PROGRAM half, not the same-OUTPUT half. GLM does not currently reproduce its own OUTPUT over 512 tokens (`docs/investigations/glm-nondeterminism.md`), and that property is a device property whose gate is `tests/determinism-glm.sh` — length-aware, because the defect is byte-identical at 32 tokens. Added 2026-08-17 with that investigation; red-proofed by a first-fit mutation of `scan_free` (hand-out collapses to all-zeros). | `crates/engine/src/fetch/asyncfetch.rs::inv_9_slot_handout_is_deterministic_under_the_layer_barrier` |