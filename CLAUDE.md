...
| long-run divergence | **OPEN defect, not root-caused.** Two witnessed sole-tenant `--ppl` pairs diverge run-to-run (up to 0.55 PPL). INV-1 **exonerated** (0 of 388,875 records); arms bit-identical in misses AND relocations before diverging, so it is a **timing race, not residency**. Window: layer *L-1*'s MoE compute / layer *L*'s attention. Repro with `--features corruption-probe --checksum-route` **under a contention witness** — the GPU flock is advisory and other agents skip it. `architecture.md` §6, `benchmarks.md` "Long-run divergence" |
...
cargo test  --release --features rocm        # 111 tests
