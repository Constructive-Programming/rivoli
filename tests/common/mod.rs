...
//! > GPU-free registry checks. A device type here would put both of them behind
//! > `--features rocm` to COMPILE — and the only CI job that exists is featureless, so the two
//! > checks that need no card would stop being checked at all.
//!
//! `dead_code` is allowed because this module is compiled into EACH test binary, and none of
//! them uses every helper. The alternative is per-consumer cfg gates on a test
