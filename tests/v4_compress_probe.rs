...
use rivoli::v4oracle::forward::{
    CompressorW, Counters, Defect, IndexerW, LayerW, Oracle, Step,
};
use rivoli::v4oracle::numerics::{bf16_decode, bf16_encode};
use rivoli::v4oracle::weights::{Checkpoint, NamedRng, V4Config, WMat};
use std::collections::HashMap;
use std::path::Path;

const CKPT: &str = "/var/db/rivoli/deepseek-v4-flash-0731";
/// `bin/v4-oracle`'s `PROMPT` tokenizes to 13 ids — the length every hole is keyed to.
const EMIT_LEN: usize = 13;
...
const PROBE_REMAINDER_LEN: usize = 300;
/// Ratio-128 decode completes its first block here: `(start_pos + 1) % 128 == 0`.
const RATIO_128_FIRST_DECODE_BLOCK: usize = 127;
...
fn compressor_w(ck: &Checkpoint, prefix: &str, ratio: usize, d: usize, rotate: bool) -> CompressorW {
...
fn indexer_w(ck: &Checkpoint, layer: usize, c: &V4Config) -> IndexerW {
    IndexerW {
        ...
        weights_proj: ck.dense(&format!("layers.{layer}.attn.indexer.weights_proj.weight")).unwrap(),
...
fn probe(name: &str, n: usize, dim: usize) -> Vec<f32> {
