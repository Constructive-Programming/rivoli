...
use rivoli::backend::hip::{
    ...
    launch_moe_expert_range_f4_situ, launch_rmsnorm_batch, launch_rmsnorm_gate_heads_f32,
    launch_rmsnorm_single, launch_short_conv_silu_f32, launch_sigmoid_gate, launch_situ_glu_f32,
};
use rivoli::v4oracle::golden::GoldenSet;

mod common;
use common::{back, dev, f32b, f32v, ok, refused, zeros};
...
/// Score every output of one operator against its golden: the operator tolerance, then the tripwire.
///
/// Five sites do exactly this — the recurrence's two outputs, the conv's two, the gated norm's one —
/// and jscpd rejected the copies at 38 and 28 tokens. Factoring it is also what stops them
/// disagreeing about the ORDER of the two bars, which matters: the tolerance is the outer envelope
/// and the tripwire is what binds, and a site that checked only one would be checking the wrong one.
fn score_all(at: &str, tol: f32, observed: f32, pairs: &[(&str, &[f32], &[f32])]) {
    for (what, got, want) in pairs {
        let r = rel(got, want);
        let at = format!("{at} {what}");
        assert!(r <= tol, "{at}: {r:e} exceeds {tol:e}");
        tripwire(r, observed, tol, &at);
    }
}

/// One defect variant has to move the result clear of the tripwire by the table's own margin.
///
/// `DEFECT_MARGIN`'s 30x, applied to the bar this fixture ENFORCES rather than to the operator
/// tolerance — which is the distinction `the_situ_sigmoid_takes_the_uncapped_gate` found the hard
/// way, where the bucket tolerance turned out unable to catch a defect the tripwire catches by
/// 2,800x. jscpd rejected the third copy of this at 47 tokens.
fn priced(at: &str, what: &str, moved: f32, observed: f32, tol: f32) {
    let bar = observed * 10.0 * 30.0;
    assert!(
        moved > bar,
        "{at} {what}: moved the operator by only {moved:e}, under the {bar:e} this fixture's \
         tripwire needs cleared by the table's 30x — so it does not price the difference it names, \
         and the agreement elsewhere says nothing about which form the kernel implements. (The \
...
/// Score the recurrence's two outputs. Both the device test and the host-oracle test call it, and
/// jscpd rejected the second copy at 48 tokens — which is the gate noticing they must not disagree
/// about scoring `state` as well as `o`. A kernel that produces the right `o` from the wrong state
/// agrees for exactly one token.
fn score_kda(salt: &str, layer: usize, tol: f32, observed: f32, got: (&[f32], &[f32]), c: &Kda) {
    // The label is built HERE rather than at the two call sites: spelled there, the two calls became
    // a 30-token match once rustfmt reflowed them — the same shared-suffix effect as one function up.
    // One line per site now, and the sites differ in the one thing that should differ.
    score_all(
        &format!("{salt} layer {layer}"),
        tol,
        observed,
...
fn the_gated_delta_recurrence_matches_the_anchor_at_every_kda_layer() {
    for_each_kda(|salt, layer, tol, lb, c| {
        ...
        score_kda(salt, layer, tol, KDA_OBSERVED_WORST, (&o, &state), &c);
    });
}
...
fn the_kda_host_oracle_agrees_with_the_anchor() {
    for_each_kda(|salt, layer, tol, lb, c| {
        ...
        // **The tripwire here is what this test lacked until 2026-08-12.** It was the only
        // golden-scored site in the file without one, and it is the site that most needs it: the five
        // red-proofs below score `moved` against a DEVICE constant, so a regression that degraded
        // THIS oracle into the 6.8e-5..6.3e-4 band would leave this test green, leave all five of
        // them green, and quietly empty the sensitivity claim they make. The constructed break that
        // found it: eps on the mean instead of the sum. Worst over both draws and layers 0/1/12:
        // 2.481e-7.
        score_kda(salt, layer, tol, 2.481e-7, (&o, &state), &c);
    });
}
        ...
        for (form, what, measured) in [
            ...
            assert!(
                ...
                "{salt} layer {layer} {what}: separation fell to {moved:e} from a measured \
                 ...
                 rather than leaning on the much weaker bar `priced` applies"
            );
            priced(&at, what, moved, KDA_OBSERVED_WORST, tol);
        }
    });
}
...
// ===================== S2 items 5b and 5c — the conv and the gated norm =====================
//
// The other two of the three boundaries fla fuses §4's ten KDA steps into. Both needed exactly ONE
// capture and both were a WEIGHT — the depthwise taps and `o_norm`'s vector — which is the fifth and
// sixth time this port has found that an input and an output do not determine an operator when a
// weight sits between them. By this point it was recognised on sight rather than by walking up to
// write the fixture and finding nothing to launch with.
//
// **Neither could have been GATED before this round, and that is the more interesting half.**
// `operator_of` bucketed both as `kda_trunk`, which has a measured floor and no ceiling: none of the
// eleven original defect runs targets a KDA projection, a conv or the norm. So each got its own
// bucket and its own defect run — `KdaConvTapsReversed` and `KdaGateBeforeNorm` — on the precedent
// item 2 set when it split `mla_attend` out of `mla`. Scoring them against a fixture tripwire alone
// is what `anchor.md` explicitly tells S2 not to do.

/// The three convolutions a KDA layer runs, and the tolerance bucket they share.
const CONVS: [&str; 3] = ["q", "k", "v"];

/// The worst relative difference `short_conv_silu_f32` shows against the anchor, over both draws,
/// all three KDA layers and all three projections — 18 `y` sites, worst at draw 1 layer 0 `v`.
///
/// The 18 `window` sites are all **exactly 0**, and that is because the window comparison is a
/// ROUND-TRIP on this fixture's own reconstruction rather than a score against the reference. See
/// `conv`: `win_in` is built by right-shifting `want_win` and dropping its last slot, and `cur` is
/// read from that same last slot — so a kernel that left-shifts and appends `cur` reproduces
/// `want_win` bit-for-bit by construction, whatever the taps say. Stated because "18 sites at
/// exactly 0" reads like strong agreement and is not that.
///
/// What it does still prove is worth keeping: the shift's DIRECTION, its distance, and which slot
/// the current token lands in — get any of those wrong and the permutation stops being the inverse
/// of the fixture's, so the miss is large. What it cannot see is the tap ORDER, which never touches
/// the window at all (`KdaConvTapsReversed` leaves it identical and moves only `y`), or the window's
/// content semantics. Those rest on two other things: `conv` ASSERTS the cache's last slot equals
/// this token's `{q,k,v}_proj` — the one equality that licenses the reconstruction and the only
/// place the reference constrains the window — and `y` is scored against `.0` at the 18 sites that
/// carry the real signal.
const CONV_OBSERVED_WORST: f32 = 1.668e-7;

/// The worst `rmsnorm_gate_heads_f32` shows, same sweep — six sites, one per draw per KDA layer.
const GATE_NORM_OBSERVED_WORST: f32 = 1.145e-7;

/// And the f64 host oracles' own agreement with the anchor, which is what
/// `the_conv_and_gated_norm_host_oracles_agree_with_the_anchor` monitors. Separate constants because
/// they are separate measurements: the device number carries a reassociated reduction that the f64
/// walk does not.
///
/// **Measured, the f64 oracle is the WORSE of the two at most sites** — 4 of 6 gated-norm sites land
/// at exactly 2x or 4x the device's ulp count, and 10 of 18 conv sites differ. Computing in f64 and
/// rounding once gets closer to the true value and *further from the anchor*, because the anchor is
/// itself an fp32 run whose accumulation order the fp32 kernel happens to track. So an oracle
/// constant can never be derived from the device one, in either direction. The conv pair coincides
/// at 1.668e-7 only because both attain their max at the same site (draw 1, layer 0, `v`) where they
/// agree to the bit; the two lists differ elsewhere.
...
///
/// **The window is reconstructed from the RETURNED cache, and the reconstruction is the fixture's
/// one inference.** `ShortConvolution` is called with every argument by keyword, so no pre-hook can
/// see its input; what the golden holds is the cache AFTER the step, whose last slot is this token's
/// projection and whose earlier slots are the previous window shifted left. So the window going IN
/// is `[anything, out[0], out[1], out[2]]` — the leading slot is shifted out and cannot affect
/// anything, which `the_conv_window_discards_only_its_oldest_slot` turns into an assertion rather
...
/// The sentinel that goes in the slot the shift discards. NaN would be caught by `rel`'s finiteness
/// check even if the kernel leaked it, but a large finite value is the stronger choice: it would
/// corrupt the output arithmetically rather than making it non-finite, so a kernel that convolved
/// the pre-shift window fails on the NUMBERS.
...
fn conv(g: &GoldenSet, layer: usize, which: &str) -> Conv {
    ...
    // The current token is the cache's LAST slot, and the reference's own projection says so — this
    // is the equality that lets the window be reconstructed at all.
...
fn host_conv(c: &Conv, reversed_taps: bool, silu: bool) -> (Vec<f32>, Vec<f32>) {
    let taps = c.taps;
    ...
    for ch in 0..channels {
        ...
        let mut acc = 0f64;
        for j in 0..taps {
            // The ONE flag per variant, as everywhere else in this file: a defect run is the correct
            // oracle with one thing changed.
            let t = if reversed_taps { taps - 1 - j } else { j };
            acc += f64::from(c.w[ch * taps + t]) * f64::from(wn[j]);
        }
        y[ch] = if silu {
...
///
/// `channels` and `taps` are parameters rather than read off the slices, because the guard test needs
/// to pass values the buffers do not agree with — and both paths going through one launch is what
/// makes that test exercise the entry point callers use. `situ_launch`'s argument, one operator over.
fn conv_launch(
    cur: &[f32],
    w: &[f32],
    win_in: &[f32],
    ...
    let (cb, wb) = (dev(&f32b(cur)), dev(&f32b(w)));
    let mut winb = dev(&f32b(win_in));
    let mut ob = zeros(cur.len().max(1) * 4);
    // The two destinations are bound before the call rather than cast inline. That is not style:
    // spelled inline, this function's last five lines were a 69-token match with `kda_launch`'s —
    // two unrelated launchers that both end with (mut, mut, null) and read two buffers back — and
    // jscpd counts a shared SUFFIX as a clone, which `launcher_names` records one file over.
    let (wp, op) = (winb.ptr_mut() as *mut f32, ob.ptr_mut() as *mut f32);
    // SAFETY: `cur`/`out` are `channels` f32, `w` and `win` are `channels x taps`, all distinct
    // allocations live for the call, as the launcher requires. The guard cases are refused before
    // any launch happens.
    unsafe {
...
fn device_conv(c: &Conv) -> (Vec<f32>, Vec<f32>) {
    ok(
        conv_launch(&c.cur, &c.w, &c.win_in, c.cur.len(), c.taps),
        "short_conv_silu_f32",
    )
}

fn for_each_conv(mut f: impl FnMut(&str, usize, &str, f32, Conv)) {
    let tol = k3_tolerance::rel_tolerance("kda_conv");
    for (salt, bytes) in GOLDENS {
        let g = load(bytes);
        for layer in KDA_LAYERS {
            for which in CONVS {
...
/// **The kernel reproduces every short convolution the anchor captured — output AND window.**
#[test]
fn the_short_conv_matches_the_anchor_at_every_kda_layer() {
    for_each_conv(|salt, layer, which, tol, c| {
        let (y, win) = device_conv(&c);
        score_all(
            &format!("{salt} layer {layer} {which}_conv1d"),
            tol,
            CONV_OBSERVED_WORST,
            &[("y", &y, &c.want_y), ("window", &win, &c.want_win)],
        );
    });
}
...
///
/// Neither is visible in the shape: the weight is `[channels][1][taps]` under either convention, and
/// a reversed filter is still a causal convolution over the same window. `KdaConvTapsReversed` is the
/// anchor's own run for the first; the second has no defect run and is priced here.
#[test]
    ...
    for_each_conv(|salt, layer, which, tol, c| {
        ...
        for (what, (y, _)) in [
            (
                ...
                host_conv(&c, true, true),
            ),
            (
                "no fused SiLU (no anchor defect)",
                host_conv(&c, false, false),
            ),
        ] {
            priced(&at, what, rel(&y, &c.want_y), CONV_OBSERVED_WORST, tol);
        }
    });
}
...
///
/// The fixture reconstructs the incoming window from the RETURNED cache, so its leading slot is a
/// sentinel rather than a captured value. That is only sound if the shift really discards it — a
/// kernel that convolved the PRE-shift window would use it, and this is what makes the
/// reconstruction an assertion instead of an assumption.
#[test]
fn the_conv_window_discards_only_its_oldest_slot() {
    for_each_conv(|salt, layer, which, _tol, c| {
        // A different sentinel in the same slot. The window is copied rather than the case moved,
        // so both launches see identical taps and identical `cur`.
        let mut other = c.win_in.clone();
        for ch in 0..c.cur.len() {
            other[ch * c.taps] = 9876.5;
        }
        let (a, wa) = ok(
            conv_launch(&c.cur, &c.w, &other, c.cur.len(), c.taps),
            "short_conv_silu_f32",
        );
...
/// One gated head norm's boundary: the recurrence's output, the gate, the weight, the result.
struct GateNorm {
    o: Vec<f32>,
    gate: Vec<f32>,
    w: Vec<f32>,
...
fn gate_norm(g: &GoldenSet, layer: usize) -> GateNorm {
    ...
    // The free cross-check `anchor.md` predicted: the module hook's input and the operator wrapper's
    // output are the same tensor under two names, so they must agree bit for bit.
    let (_, kda_o) = float(
        g,
...
/// How the norm and the gate compose. **Four variants and each is a documented alternative.**
#[derive(Clone, Copy, PartialEq)]
enum GateForm {
    Reference,
    ...
    SiluGate,
    /// `eps` on the SUM of squares, which is the recurrence's L2 convention one operator over.
    EpsOnSum,
}
...
fn host_gate_norm(c: &GateNorm, eps: f32, form: GateForm) -> Vec<f32> {
    ...
    for h in 0..c.heads {
        ...
        let denom = if form == GateForm::EpsOnSum {
            sq + f64::from(eps)
        } else {
            sq / d as f64 + f64::from(eps)
        };
        let inv = 1.0 / denom.sqrt();
        for (k, i) in r.enumerate() {
            let gate = if form == GateForm::GateFirst {
                1.0
            } else {
                sig(c.gate[i])
            };
...
/// One launch, dims and `eps` as parameters so the guard test drives the same entry point.
fn gate_norm_launch(
    c: &GateNorm,
    heads: usize,
    head_dim: usize,
    ...
    let mut out = zeros(c.o.len() * 4);
    // SAFETY: `o`, `gate` and `out` are `heads x head_dim` f32 and `weight` is `head_dim`, all
    // distinct allocations live for the call. The guarded dims are refused before any launch.
    unsafe {
...
fn for_each_gate_norm(mut f: impl FnMut(&str, usize, f32, f32, GateNorm)) {
    let tol = k3_tolerance::rel_tolerance("kda_gate_norm");
    for (salt, bytes) in GOLDENS {
        let g = load(bytes);
        // `FusedRMSNormGated(self.head_dim, eps=config.rms_norm_eps, activation='sigmoid')` — the
        // reference passes the config's value explicitly. fla's own default is ALSO 1e-5, so reading
        // either would score green here; this reads the config because that is what the reference
        // passes, and the coincidence is exactly what would hide a wrong source.
        let eps = eps(&g);
        for layer in KDA_LAYERS {
...
/// The one scored buffer of the gated norm, named once. Two callers score it — the kernel and the
/// host oracle — against the same `want` at the same site, which is 33 tokens of identical call and
/// so a build error before it is a style question.
fn score_gate_norm(at: &str, tol: f32, observed: f32, got: &[f32], c: &GateNorm) {
    score_all(at, tol, observed, &[("out", got, &c.want)]);
}
...
fn the_gated_head_norm_matches_the_anchor() {
    for_each_gate_norm(|salt, layer, tol, eps, c| {
        ...
        score_gate_norm(&at, tol, GATE_NORM_OBSERVED_WORST, &got, &c);
    });
}

/// **Norm THEN gate, `sigmoid` not `silu`, and `eps` on the mean — all three priced.**
///
/// The first is trap 10 and has the anchor's own run (`KdaGateBeforeNorm`); it is not a permutation
/// of two elementwise multiplies, because gating first changes the RMS the norm divides by. The other
/// two have no defect run: `silu` is the other activation fla accepts at this call site, and eps on
/// the SUM is the convention the recurrence's L2 norm uses **in the same layer**, a few hundred lines
/// away in the same kernel file.
#[test]
fn the_gate_ordering_and_the_norm_convention_are_priced() {
    for_each_gate_norm(|salt, layer, tol, eps, c| {
        ...
        for (what, form) in [
            ...
            let moved = rel(&host_gate_norm(&c, eps, form), &c.want);
            priced(&at, what, moved, GATE_NORM_OBSERVED_WORST, tol);
        }
    });
}

/// **Both host oracles agree with the anchor, which is what lets their variants mean anything.**
///
/// The six red-proofs in this section perturb the ORACLE and assert the perturbation moves the
/// result. That argument is empty unless the unperturbed oracle agrees to the same order the kernel
/// does — the gap a review found in item 5a's equivalent, where the oracle underwriting five
/// red-proofs was monitored at 2,780x slack. One test for both operators: it is the same argument
/// twice, and two copies of it is what jscpd is for.
#[test]
fn the_conv_and_gated_norm_host_oracles_agree_with_the_anchor() {
    for_each_conv(|salt, layer, which, tol, c| {
        let (y, win) = host_conv(&c, false, true);
        score_all(
            &format!("oracle {salt} layer {layer} {which}_conv1d"),
            tol,
            CONV_ORACLE_WORST,
            &[("y", &y, &c.want_y), ("window", &win, &c.want_win)],
        );
    ...
    for_each_gate_norm(|salt, layer, tol, eps, c| {
        let at = format!("oracle {salt} layer {layer}");
        let got = host_gate_norm(&c, eps, GateForm::Reference);
        score_gate_norm(&at, tol, GATE_NORM_ORACLE_WORST, &got, &c);
    });
}

/// One synthetic case per operator, at a width the caller picks. `want` is empty in both: these are
/// scored against the f64 host oracles, not against the golden, so there is nothing to carry.
fn synthetic_conv(channels: usize, taps: usize, seed: u64) -> Conv {
    let mut r = common::Lcg(seed);
    let mut draw = |len: usize| (0..len).map(|_| r.f()).collect::<Vec<f32>>();
    Conv {
...
fn synthetic_gate_norm(heads: usize, head_dim: usize, seed: u64) -> GateNorm {
    let mut r = common::Lcg(seed);
    let mut draw = |len: usize| (0..len).map(|_| r.f()).collect::<Vec<f32>>();
    GateNorm {
...
///
/// This was the one coverage gap items 1-5a each closed and 5b/5c had not: every golden-backed test
/// above runs at the anchor's tiny widths, and for these two kernels that is not a smaller version of
/// the real case, it is a different code path.
///
/// **The conv's grid is never exercised by the goldens at all.** 128 channels is ONE block of 256, so
/// `blockIdx.x` is always 0 and the `c >= channels` tail is the only thing under test. The real width
/// is 12,288 channels — 48 blocks — and a `blockIdx.x * blockDim.x` mistake is invisible until then.
/// `257` is here for the awkward case a round number hides: two blocks whose second has one live
/// thread, which is where an off-by-one in the tail and an off-by-one in the grid disagree.
///
/// **The gated norm's reduction gets two more halvings.** `block_sum_lds` at `blockDim.x = 128` runs
/// a 7-level ladder against the anchor's 5 and takes 4x the LDS, and the launcher computes that LDS
/// size itself — so a kernel indexing `red` beyond what the launcher allocated shows up here first.
/// `heads = 1` and `head_dim = 2` pin the two degenerate ends the guards still admit.
///
/// Scored against the same f64 host oracles the golden-backed tests use, which is what makes this a
/// second measurement rather than a second copy of the first.
#[test]
fn the_conv_and_gated_norm_hold_at_real_widths() {
    // `q`/`k`/`v` are each `num_heads * head_dim` = 96 x 128 channels at the real config, and `taps`
    // is `short_conv_kernel_size` = 4. The two small cases share `taps` with the real one, so a
    // failure separates the grid from the window arithmetic.
    for &(channels, taps) in &[(12288usize, 4usize), (257, 4), (1, 2), (12288, 16)] {
        ...
        let (y, win) = device_conv(&c);
        let (wy, ww) = host_conv(&c, false, true);
        // 10x the 2.822e-7 measured over these four cases, whose worst is `y` at 12288x16 — the
        // deepest window, as expected: a 16-tap dot is four times the reduction depth and the device
        // sums it in a different order from the host's f64 walk. The 4-tap cases sit at 1.2-1.5e-7,
        // i.e. at the 1.668e-7 the golden-backed sites measure, so the extra slack is bought by the
        // tap count and by nothing else. `1x2` is EXACTLY 0: two terms leave f32 and f64 nothing to
        // disagree about. The window is exact at every case — it is a copy, not arithmetic — so it
        // gets no slack at all, and a nonzero there is a shift bug rather than rounding.
        let dy = rel(&y, &wy);
    ...
    // `head_dim` 128 x 96 heads is the real geometry; `(1, 128)` isolates the head count from the
    // width, and `(96, 2)` is the shallowest ladder the power-of-two guard admits.
    for &(heads, head_dim) in &[(96usize, 128usize), (1, 128), (96, 2), (4, 1024)] {
        ...
        // 10x the 1.913e-7 measured over these four cases. The worst is the REAL geometry, 96x128 —
        // not the 1024-wide case, which reaches 1.543e-7 on a ladder two levels deeper. So the
        // separation is not simply growing with reduction depth here the way the conv's does with
        // tap count, and a comment predicting otherwise was wrong before it was measured.
        let d = rel(&got, &want);
...
/// **Both launchers refuse what they cannot compute.**
#[test]
fn the_conv_and_gated_norm_guard_their_shapes() {
    let w = vec![0.5f32; 8 * 4];
    let (cur, win) = (vec![1.0f32; 8], vec![0.0f32; 8 * 4]);
    let launch_conv = |channels: usize, taps: usize| conv_launch(&cur, &w, &win, channels, taps);
    refused(launch_conv(0, 4), 1001, "channels 0");
    // `taps = 1` is a per-channel scale, not a convolution — the shape a caller reaches by reading
    // `short_conv_kernel_size` as the history length rather than the window.
    refused(launch_conv(8, 1), 1002, "taps 1");
    refused(launch_conv(8, 17), 1002, "taps 17");
    ...
    let launch_norm =
        |heads: usize, head_dim: usize, eps: f32| gate_norm_launch(&c, heads, head_dim, eps);
    refused(launch_norm(0, 32, 1e-5), 1001, "heads 0");
    refused(launch_norm(2, 0, 1e-5), 1001, "head_dim 0");
    // 2048 is a power of two, so it passes 1003 and only the block-size bound stops it. Without this
    // case nothing reached 1002 at all: every other refusal here fires an earlier guard, and a test
    // that only asserts `is_err()` cannot tell which one did.
    refused(launch_norm(2, 2048, 1e-5), 1002, "head_dim 2048");
    // 96 is K3's HEAD COUNT, so a transposed argument pair lands exactly here — the same case the
    // recurrence's guard test uses, for the same reason.
    refused(launch_norm(2, 96, 1e-5), 1003, "head_dim 96");
    for bad in [-1.0f32, f32::NAN, f32::INFINITY] {
        refused(launch_norm(2, 32, bad), 1006, &format!("eps {bad}"));
    }
    // Zero eps is LEGAL — it is what an exact RMSNorm is — so the refusals above are not "rejects
    // everything".
