...
`tojson` -- and writes `(name, kwargs, expected)` triples to `glimmer-chat-cases.json` beside
itself. `glimmer_template.rs`, also beside it, reads them with no network, no GPU and no Python.
...
same author. A pin whose expected side comes from the model's own file cannot share a
misreading with the port.

**PORT NOTE 2026-08-16.** Vendored into the rewrite from `old:tests/glimmer_template_driver.py`
(`wt/glimmer-s2` @ 6b7f496), byte-identical apart from the paths in this docstring: it now
lives in `crates/artifact/tests/` and resolves its output beside itself, so the program is
unchanged and only its own instructions moved. The `docs/investigations/` reference above and
the two `glimmer_*.rs` decode gates it cited are the OLD tree's; the rewrite's engine loop, and
therefore its equivalents, arrive after M7.
...
Usage (no GPU, no lock, no wrapper script -- it is one command and takes seconds):
    ...
    /home/rhansen/glimmer-anchor/venv/bin/python \\
        crates/artifact/tests/glimmer_template_driver.py
