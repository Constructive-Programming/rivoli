...
/// The reference's two encode caps, as one value.
///
/// **A struct because they are a PAIR and because both are `usize`.** `_encode_text_piece` nests
/// them — the outer slices characters, the inner run counter restarts inside each slice — so they
/// are never meaningfully chosen apart, and two adjacent bare `usize` parameters are the silent
/// call-site swap `format::meta::ArtifactDirs` exists to prevent (its doc carries the argument).
/// Swapping these would not fail to compile and would not fail any short fixture: with
/// `chars = 25_000, run = 400_000` the inner cap simply never trips and ordinary prompts tokenize
/// identically. It also keeps this module out of the Primitive Obsession band CodeScene records
/// (12 primitive arguments across the methods, against a threshold of 11).
#[derive(Debug, Clone, Copy)]
pub struct EncodeCaps {
    /// Characters per outer slice — [`MAX_ENCODE_CHARS`].
    pub chars: usize,
    /// Consecutive same-class characters per inner chunk — [`MAX_SAME_CLASS_RUN`].
    pub run: usize,
}

impl Default for EncodeCaps {
    /// The shipped pair. `Default` rather than a `const`, so a caller writing
    /// `EncodeCaps::default()` gets the reference's values and only a test spells its own.
    fn default() -> Self {
        Self {
            chars: MAX_ENCODE_CHARS,
            run: MAX_SAME_CLASS_RUN,
        }
    }
}
...
impl Vocab {
    ...
    pub fn encode(&self, text: &str) -> Result<Vec<u32>> {
        self.encode_capped(text, EncodeCaps::default())
    }
    ...
    /// other than [`EncodeCaps::default`].
    pub fn encode_capped(&self, text: &str, caps: EncodeCaps) -> Result<Vec<u32>> {
        let EncodeCaps { chars, run } = caps;
        ensure!(chars > 0 && run > 0, "both caps must be positive");
        let mut out = Vec::new();
        ...
        for outer in char_chunks(text, chars) {
            for chunk in split_same_class_runs(outer, run) {
...
// Unit tests live in `tiktoken/tests.rs`, following `v4_encoding`'s split: this file crossed the
// 800-line soft cap and the gate's instruction is that the next edit here shrink it. The tests are
// the part that can move without splitting an argument across files.
#[cfg(test)]
#[path = "tiktoken/tests.rs"]
mod tests;
