//! MoE routing, host-pure — INV-1's subject. `route_into` is a pure function of
//! `(gate logits, bias, top_k, scoring)`; the residency layer cannot even be named from
//! this crate, so "routing never consults the cache" is enforced by the DAG before any
//! test asserts it. Ported from the old tree's `math.rs` routing half (`wt/glimmer-s2`
...
mod tests {
    use super::*;
    use crate::num::{
        bf16_to_f32, e4m3_to_f32, f32_to_bf16, f32_to_e4m3, f32_to_f16, sigmoid, sqrt_softplus,
    };
