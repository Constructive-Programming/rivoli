...
//!
//! `Attention.forward` and everything reached only from it — the two index-list
//! constructors, the online softmax, the six attention stages — moved VERBATIM to
//! [`crate::v4oracle::attention`] on 2026-08-15. The reason was mechanical, not conceptual:
//! this file had 1068 code lines against CodeScene's file-size cliff measured at ~880 and
//! scored 9.09, and `crates/cli/tests/codescene.rs` gates the whole tree at 10/10. The cut
//! ran along the one seam that already existed — `run_layer` calls `attention` and threads
//! `Sinks` — so `wrow`, `round_bf16`, `rmsnorm`, `rope_row` and `kv_act_quant` are now
//! `pub(super)`. That is module-internal reach, not public API: [`window_topk`] and
//! [`compress_topk`] are re-exported below at their original paths, and nothing else visible
//! from outside this crate moved.
...
/// The selection constructors kept at their original path.
///
/// They are `pub` for S2c's isolated compressor/indexer drives and are named in
/// `tests/v4_oracle.rs` by full path (`v4oracle::forward::window_topk`), so the re-export is
/// what makes the 2026-08-15 file split invisible to every caller rather than a rename with
/// extra steps.
