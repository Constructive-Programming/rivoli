...
fn fp8(name: &str, rows: usize, cols: usize, scale: f32) -> WMat {
    ...
    for br in 0..sr {
        for bc in 0..sc {
            ...
            for r in r0..r1 {
                for c in c0..c1 {
                    amax = amax.max(v[r * cols + c].abs());
                }
            }
            let (code, sv) = block_scale(amax, FP8_MAX);
            s[br * sc + bc] = code;
            for r in r0..r1 {
                for c in c0..c1 {
                    w[r * cols + c] = e4m3_encode((v[r * cols + c] / sv).clamp(-FP8_MAX, FP8_MAX));
                }
            }
        }
    }
...
/// Quantize to fp4 with one e8m0 scale per 32 elements of K, packed two nibbles per byte
    ...
    let layers = (0..c.n_layers)
        .map(|l| {
            ...
            LayerW {
                ...
                q_norm: draw(&format!("{p}.q_norm"), c.q_lora_rank, 0.3)
                    .iter()
                    .map(|x| 1.0 + x)
                    .collect(),
                wq_b: fp8(
                ...
                compressor: (ratio != 0)
                    .then(|| compressor(&format!("{p}.comp"), c, ratio, c.head_dim, false)),
                indexer: (ratio == 4).then(|| IndexerW {
                    wq_b: fp8(
                        &format!("{p}.idx.wq_b"),
