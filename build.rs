...
/// Heads per attention workgroup, mirroring `HB` in `kernels/attn.hip`. Injected here for
/// the same reason as the two above: it was spelled `#define HB` in the shader AND
/// `const MLA_HB` in `src/backend/vk.rs`, with a `MLA_HB * MLA_SUBW == BLOCK` assert as the
/// only guard — and that assert held only by coincidence, because `BLOCK` is
/// `ROWS_PER_BLOCK * WAVE` and both products were 256. Raising HB to 16 (2026-08-02) broke
/// the coincidence and would have left the coupling unchecked, which is exactly the
/// "declared twice with a must-match comment and nothing checking" regression this block
/// was written to prevent. Note the attention workgroup is now `VK_MLA_HB * VK_WAVE` = 512
/// threads, which is NOT `BLOCK` — see `missing_features`, which checks both.
const VK_MLA_HB: u32 = 16;
    ...
    std::fs::write(
        ...
        format!(
            "// @generated by build.rs — the single source is VK_WAVE/VK_ROWS_PER_BLOCK there.\n\
             ...
             pub const MLA_HB: usize = {VK_MLA_HB};\n"
        ),
    )
