---
name: amd-tracelens-analysis-orchestrator
description: >-
...
> **HOUSE CONSTRAINT (rivoli, added at vendor time — not upstream).** Every device command
> below runs ONLY under the `gpu-measure` skill's lease + `flock /var/run/sys-gpu.lock` +
> per-arm contention witness; the GPU is sole-tenant and this hook-guarded harness refuses an
> unlocked one. This box is **gfx1151 / Strix Halo, LPDDR5 unified memory through GTT** — not
> Instinct, no HBM, no tensor parallelism — so every roofline denominator, peak-bandwidth band
> and `efficiency_percent` this skill can produce is ANOTHER MACHINE'S number and is not
> citeable here. Read `PROVENANCE.md` in this directory before acting on anything below: it
> records the pin, the review verdict, what is neutralized, and the earn-or-drop clause.
